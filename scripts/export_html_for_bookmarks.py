...
def main():
    ...
    parser.add_argument('bookmarks_dir', metavar='bookmarks-dir', type=path.abspath, help='TODO')
    ...
    for root, __, files in os.walk(args.bookmarks_dir):
        ...
        file_paths_by_mtime_desc = sorted(file_paths_by_mtime_desc, key=operator.itemgetter(1), reverse=True)
        
        for file_path, __ in file_paths_by_mtime_desc:
            try:
                title = unicode(path.splitext(path.basename(file_path))[0], errors='strict')
            except:
                print(file_name)
                raise
            ...
            with open(file_path) as f:
                ...
                if match is None:
                    raise RuntimeError('could not find url in shortcut file {0}'.format(file_path))
